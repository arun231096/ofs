...
rsp.getData = function(field){
    if (field === 'person') {
        if (!personData) {
            service.doGet('GET','../../ws.ccc/do/person?field=readall', false);
        ...
        if (!addressData) {
            service.doGet('GET','../../ws.ccc/do/address?field=readall', false);
...
rsp.onselectedListPerson = function (row) {
    ...
    inputs[6].setAttribute('type', 'hidden');
    inputs[7].setAttribute('type', 'hidden');
    inputs[8].setAttribute('type', 'hidden');
}
...
rsp.addPerson = function() {
    ...
    console.log(rsp.jsonPerson);
    personData.push(rsp.jsonPerson);
    rsp.getData('person');
    rsp.setViewPanel('person');
    rsp.listenEvents('person');
    rsp.setDefault('person');
